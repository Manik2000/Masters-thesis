...
if __name__ == "__main__":
    ...
    detectors = [BaseDetector(window_length=20)] + \
        [CPDetector(model_path=os.path.join("models", f"cnn_lstm_attention_{window}.keras"), 
                    window_length=window) for window in [10, 15, 20]]
    ...
    for traj_length in tqdm(traj_lengths, desc="Trajectory length"):
        ...
        with open(os.path.join(path, f"{traj_length}", "test", "change_points.pkl"), "rb") as f:
            change_points = [np.array(i) for i in pickle.load(f)]
        with open(os.path.join(path, f"{traj_length}", "test", "labels.pkl"), "rb") as f:
            labels = pickle.load(f)
        for idx, traj in tqdm(enumerate(data), desc="Trajectories cp", total=data.shape[0]):
    ...
    results_df.to_csv(os.path.join("data", "results", "final_eval_results.csv"), index=False)
