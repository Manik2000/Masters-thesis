...
def set_up_parser():
    ...
    parser.add_argument("-length", type=int, default=85, help="Length of the trajectory")
...
if __name__ == "__main__":
    ...
    model_paths = [
        os.path.join("models", f"cnn_lstm_attention_{window_length}.keras") for window_length in window_lengths 
    ]
    END = [length]

    all_results = {}

    for window_length, model_path in tqdm(zip(window_lengths, model_paths), desc="Evaluating models", total=len(window_lengths)):
        ...
        for threshold in tqdm(thresholds, desc="Threshold"):
            ...
            preds = [cp_det.get_change_points_preds(prob_seq_i) + END for prob_seq_i in prob_seq]
            f1_scores.append(np.mean([f1_score(np.array(change_points[i]), np.array(preds[i])) for i in range(len(preds))]))
            rmse_scores.append(np.mean([rmse(np.array(change_points[i]), np.array(preds[i])) for i in range(len(preds))]))
