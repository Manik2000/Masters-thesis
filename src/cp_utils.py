...
def merge_changepoints(changepoints, window_length):
    ...
    return np.array(
        [round(np.mean(changepoints[labels == i])) for i in np.unique(labels)]
    )
...
def create_window_dataset(trajectory, changepoints, window_length):
    ...
    for i in range(len(trajectory) - window_length):
        ...
        y.append(
            1 if any(cp in range(i, i + window_length) for cp in changepoints) else 0
        )
...
def create_dataset(trajs, labels, window_length):
    ...
    y = []
    changepoints = [
        get_changepoints(labels[:, i, :], window_length=window_length)
