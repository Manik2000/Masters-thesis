...
class AndiDataset:
    ...
    def generate_proper_alpha():
        alpha_1, alpha_2 = (
            AndiDataset.random_alpha_value(),
            AndiDataset.random_alpha_value(),
        )
    ...
    def get_alphas_and_Ds(self):
        ...
        return [
            [alpha_1, np.random.uniform(0, 0.15)],
            [alpha_2, np.random.uniform(0, 0.15)],
        ], [[D_1, np.random.uniform(0, 0.15)], [D_2, np.random.uniform(0, 0.15)]]

    def single_state(self, N, T, alphas, Ds):
        return self.model_phenon.single_state(
            N=N, T=T, alphas=alphas, Ds=[Ds, np.random.uniform(0.01, 0.4)]
        )

    def multi_state(self, N, T, alphas, Ds):
        p = np.random.uniform(0.02, 0.15)
        p2 = np.random.uniform(0.02, 0.15)
        return self.model_phenon.multi_state(
            N=N, T=T, M=[[1 - p, p], [p2, 1 - p2]], Ds=Ds, alphas=alphas
        )

    def confinemnet(self, N, T, alphas, Ds):
        compartments_center = models_phenom._distribute_circular_compartments(
            Nc=self.number_compartments, r=self.radius_compartments, L=self.L
        )
        return self.model_phenon.confinement(
            N=N,
            T=T,
            Ds=Ds,
            alphas=alphas,
    ...
    def immobile(self, N, T, alphas, Ds):
        ...
        return self.model_phenon.immobile_traps(
            N=N,
            T=T,
            L=self.L,
            r=2,
    ...
    def dimmerization(self, N, T, alphas, Ds):
        ...
        return self.model_phenon.dimerization(
            N=N, L=self.L, T=T, alphas=alphas, Ds=Ds, r=1, Pb=Pb, Pu=Pu
        )
